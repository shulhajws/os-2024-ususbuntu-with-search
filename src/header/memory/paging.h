 ...
 * @param dirty_bit         Indicate whether software has written to the 4-MByte page referenced by this entry
 * @param use_pagesize_4_mb Must be 1 (otherwise, this entry references a page table)
 * 
 */
struct PageDirectoryEntryFlag {
    uint8_t present_bit        : 1;
    // TODO : Continue. Note: Only 8-bit flags
    uint8_t write_bit          : 1; // read/write
    uint8_t user_bit           : 1; // user/supervisor
    uint8_t pwt_bit            : 1; // page-level write-through
    uint8_t pcd_bit            : 1; // page-level cache disable
    uint8_t accessed_bit       : 1; // accessed
    uint8_t dirty_bit          : 1; // dirty
    uint8_t use_pagesize_4_mb  : 1; // page size
 ...
 * @param higher_address  Bits (M–1):32 of physical address of the 4-MByte page referenced by this entry (in this case 39:32)
